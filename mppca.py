...
def mppca_gem(X, pi, mu, W, sigma2, niter, tol=1e-4):
    N, d = X.shape
    p = len(sigma2)
    _, q = W[0].shape

    sigma2hist = np.full((p, niter),np.nan)
    M = np.zeros((p, q, q))
    Minv = np.zeros((p, q, q))
    Cinv = np.zeros((p, d, d))
    logR = np.zeros((N, p))
    ...
    L = np.full(niter,np.nan)
    for i in range(niter):
        #print('.', end='')
        if i==0: print('MPPCA iteration:', end='')        
        ...
        if (i>0) and (np.abs(1-L[i]/L[i-1]) < tol):
            break
